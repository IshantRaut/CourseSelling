...
const CourseContainer = () => {
  ...
  return (
    <div className="max-w mx-auto px-4 py-8 bg-gray-900 min-h-screen">
      <h2 className="text-3xl font-bold text-white mb-6 text-center">Available Courses</h2>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center bg-gray-800 p-4 rounded-lg shadow-md">
        ...
        <input
          ...
          className="mt-4 md:mt-0 px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 bg-gray-700 text-white placeholder-gray-400"
        />
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mt-6">
        {filteredCourses.length > 0 ? (
          filteredCourses.map((course) => (
            <div key={course.id} className="bg-gray-800 shadow-md rounded-lg overflow-hidden text-white">
          ...
          <p className="text-gray-400 text-center col-span-4">No courses found.</p>
        )}
      </div>
    </div>
  );
